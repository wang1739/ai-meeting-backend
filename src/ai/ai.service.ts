...
export class AiService {
  ...
  async generateSummary(meetingId: string, userId: string) {
    ...
    const [savedSummaries, savedActionItems] = await this.prisma.$transaction(async (tx) => {
      ...
      if (summaryData.actionItems.length > 0) {
        // 1. 查询数据库中该会议已有的行动项（防止并发/重复请求导致重复写入）
        ...
        // 2. 行内去重：前50字符相似判定
        const uniqueInPayload = summaryData.actionItems.filter(
          (item, index, self) =>
            index === self.findIndex((t) => t.description.slice(0, 50) === item.description.slice(0, 50)),
        );
        ...
        console.log(
          `[AiService] 去重统计: 原始=${summaryData.actionItems.length}, 行内去重后=${uniqueInPayload.length}, DB去重后=${newItems.length}, DB已有=${existingDbItems.length}`,
        );
  ...
  private buildPrompt(title: string, agenda: string, transcript: string): string {
...
请严格按照以下 JSON 格式返回，不要包含其他内容：
{
  ...
  /**
   * 解析议程字段：如果是 JSON 字符串数组，解析为可读文本
   */
  private parseAgenda(agenda: any): string {
    if (!agenda) return '';
  ...
  private getMockSummary(title: string, agendaText: string, transcriptText: string) {
    ...
    // 含具体产出物的通用行动项（最多5条）
    let actionItems: Array<{ description: string; assignee: string; dueDate: string }> = [];
    ...
    if (hasTranscript) {
      ...
      const specificTasks = [
        (item: string) => `输出「${item}」的实施方案文档，包含执行计划、资源配置和时间节点，提交团队评审`,
        (item: string) => `完成「${item}」的调研工作并输出调研报告，附带相关数据分析和建议方案`,
        (item: string) => `制定「${item}」的执行细则和操作流程，同步至所有相关成员`,
      ];
