...
# 1. Remove broken prisma.config.* files (Prisma works without them)
print('\n[1] Remove broken prisma.config, reinstall...')
ec, out = run("""
export PATH=/usr/local/bin:$PATH
cd /root/ai-meeting-backend
rm -f prisma.config.js prisma.config.ts
# Also remove any build cache
